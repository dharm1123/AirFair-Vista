...
# ─────────────────────────────────────────────────────────────────────────────
#  JAVASCRIPT — Minimal selectbox styler
#  Theme is now locked via .streamlit/config.toml, so no theme detection
#  needed. This only ensures selectbox dropdowns render consistently.
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("""
<script>
(function() {
    ...
    function styleSelectboxes() {
        ...
        document.querySelectorAll('div[data-baseweb="select"]').forEach(function(sel) {
            ...
            if (box) {
                ...
                box.style.setProperty('border', '1.5px solid #c8d6f0', 'important');
            }
            sel.querySelectorAll('[class*="singleValue"],[class*="placeholder"]').forEach(function(el) {
        ...
        // ── Sidebar selectboxes (always dark) ─────────────────────
        document.querySelectorAll(
            '[data-testid="stSidebar"], [data-testid="stSidebar"] > div, ' +
            '[data-testid="stSidebar"] > div > div, [data-testid="stSidebarContent"]'
        ).forEach(function(el) {
            el.style.setProperty('background-color', '#09122c', 'important');
        });
    }

    styleSelectboxes();
    var observer = new MutationObserver(styleSelectboxes);
