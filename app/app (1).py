"""
AirFair-Vista — app.py  (Enhanced Version)
==========================================

IMPROVEMENTS OVER ORIGINAL:
  1. Dynamic Source → Destination routing via VALID_DESTINATIONS dict
     (was hardcoded single route; now fully data-driven)
  2. Round-trip / return journey support with:
       - Trip-type toggle (One-way / Round-trip)
       - Return date picker with validation (must be > departure date)
       - Return flight prediction & dedicated ticket card
       - Combined total fare display

ARCHITECTURE:
  preprocessor.py  ← feature engineering, lookups, pure functions
  app.py           ← Streamlit UI (this file); imports from preprocessor
...
# ─────────────────────────────────────────────────────────────────────────────
#  PATH SETUP — import preprocessor from backend folder
# ─────────────────────────────────────────────────────────────────────────────
...
def _city_code(city: str) -> str:
    return CITY_CODE_MAP.get(city, str(city)[:3].upper())


def _stops_display(stops_value: str) -> str:
    return STOPS_DISPLAY_MAP.get(stops_value, str(stops_value).replace("_", " ").title())


def render_html(html: str) -> None:
    """Render HTML reliably — uses st.html() (Streamlit >= 1.31) which renders
    HTML directly without any Markdown processing or sanitisation.
    Falls back to st.markdown(unsafe_allow_html=True) for older Streamlit builds.
    """
    cleaned = textwrap.dedent(html).strip()
    try:
        # st.html() was introduced in Streamlit 1.31 and is the preferred API
        # for raw HTML. It never treats content as Markdown, so indented HTML
        # is rendered correctly and never shown as a code block.
        st.html(cleaned)
    except AttributeError:
        # Older Streamlit (< 1.31) — fall back to markdown with unsafe HTML
...
def predict_price(airline, source, destination, stops,
    ...
    if not MODEL_LOADED or model is None:
        raise FileNotFoundError(
            'new_dataset_flight_price_prediction_pipeline.pkl is required before prediction.'
        )
    ...
    if raw < 15:       # log-transformed target → invert
        raw = np.expm1(raw)
...
# ─────────────────────────────────────────────────────────────────────────────
#  MASTER CSS — theme-aware; no hardcoded light-only colours
# ─────────────────────────────────────────────────────────────────────────────
st.markdown("""
<style>
...
.stCheckbox > label, .stMultiSelect > label {
    ...
    margin-bottom: 4px !important; color: var(--text-h) !important;
}

/* ─── SELECTBOX ─────────────────────────────────────────────────────────── */
div[data-baseweb="select"] > div:first-child {
    border-radius: var(--radius-sm) !important;
    min-height: 42px !important; transition: box-shadow 0.15s ease !important;
}
...
div[data-baseweb="select"] input {
    color: var(--text-h) !important; -webkit-text-fill-color: var(--text-h) !important;
}
...
[data-testid="stPlotlyChart"] > div {
    background: var(--card-bg) !important; border-radius: var(--radius-md) !important;
    padding: 6px !important; border: 1px solid var(--card-border) !important;
    box-shadow: var(--card-shadow) !important;
}
...
button[kind="primary"] {
    ...
    padding: 14px 28px !important; transition: transform 0.1s, box-shadow 0.1s !important;
}
button[kind="primary"]:hover {
    transform: translateY(-1px) !important;
    box-shadow: 0 8px 24px rgba(0,82,204,0.35) !important;
}
hr { border: none !important; border-top: 1px solid var(--card-border) !important; margin: 20px 0 !important; }
...
.page-header {
    ...
    border-radius: var(--radius-xl); padding: 36px 40px 32px; margin-bottom: 28px;
    position: relative; overflow: hidden; box-shadow: 0 8px 32px rgba(0,82,204,0.28);
}
.page-header::before {
    content: '✈'; position: absolute; right: 24px; top: 50%;
    transform: translateY(-50%); font-size: 5.2rem; opacity: 0.035;
    line-height: 1; pointer-events: none;
}
.page-header h1 {
    font-family: 'Syne', sans-serif; font-size: 2.2rem; font-weight: 900;
    margin: 0 0 6px; color: #fff; letter-spacing: -0.5px;
}
...
.section-title {
    ...
    text-transform: uppercase; color: var(--primary); margin: 0 0 16px;
...
.output-section-title {
    ...
    color: var(--text-h); margin: 28px 0 14px;
    display: flex; align-items: center; gap: 8px;
}
.output-section-title::after {
    content: ''; flex: 1; height: 1.5px;
    background: linear-gradient(90deg, var(--primary-tint), transparent); margin-left: 8px;
}
...
.trip-btn {
    flex: 1; padding: 10px 18px; border-radius: var(--radius-md); font-size: 0.85rem;
...
/* ─── RETURN BANNER ─────────────────────────────────────────────────────── */
.return-banner {
    background: linear-gradient(135deg, #0d1f5c 0%, #0052cc 100%);
    border-radius: var(--radius-md); padding: 14px 20px; margin: 14px 0;
    display: flex; align-items: center; gap: 14px;
    border-left: 4px solid var(--accent);
}
.return-banner-icon { font-size: 1.5rem; }
.return-banner-text { color: white; font-size: 0.82rem; font-weight: 600; }
.return-banner-sub  { color: rgba(255,255,255,0.65); font-size: 0.72rem; margin-top: 2px; }

/* ─── TOTAL FARE CARD ───────────────────────────────────────────────────── */
.total-fare-card {
    background: linear-gradient(135deg, #09122c 0%, #0d1f5c 60%, #0052cc 100%);
    border-radius: var(--radius-xl); padding: 28px 36px; margin: 20px 0;
    display: flex; justify-content: space-between; align-items: center;
    box-shadow: 0 8px 32px rgba(0,82,204,0.30);
    border: 1px solid rgba(255,255,255,0.08);
}
.total-fare-label  { font-size: 0.68rem; color: rgba(255,255,255,0.60); font-weight: 700; letter-spacing: 1.5px; text-transform: uppercase; }
.total-fare-amount { font-family: 'Syne', sans-serif; font-size: 3rem; font-weight: 900; color: white; letter-spacing: -1px; line-height: 1; }
.total-fare-sub    { font-size: 0.78rem; color: rgba(255,255,255,0.65); margin-top: 6px; }
.total-fare-badge  {
...
.valid-badge {
    ...
    color: #16a34a; border-radius: 20px; padding: 2px 10px;
    font-size: 0.68rem; font-weight: 700; margin-top: 4px;
}
[data-theme="dark"] .valid-badge { color: #4ade80; }
.duration-chip {
    display: inline-flex; align-items: center; gap: 8px;
    background: var(--primary-tint); border: 1.5px solid rgba(0,82,204,0.20);
    border-radius: var(--radius-sm); padding: 10px 16px;
    font-size: 0.84rem; font-weight: 600; color: var(--primary); margin: 8px 0 4px; width: 100%;
}
...
/* ─── LIVE ESTIMATE CARD ────────────────────────────────────────────────── */
.live-card {
    background: var(--primary-tint); border: 1.5px solid rgba(0,82,204,0.22);
    border-radius: var(--radius-md); padding: 14px 20px;
    display: flex; align-items: center; gap: 18px; flex-wrap: wrap; margin-bottom: 4px;
}
.live-card-label { font-size: 0.62rem; font-weight: 800; color: #3b82f6; text-transform: uppercase; letter-spacing: 1.5px; }
.live-card-price { font-family: 'Syne', sans-serif; font-size: 1.7rem; font-weight: 900; color: var(--primary); letter-spacing: -1px; }
...
/* Return ticket gets a slightly different accent line */
.ticket-header.return-header::after {
    background: linear-gradient(90deg, #22c55e, #4ade80, #22c55e);
}
.ticket-airline {
    font-family: 'Syne', sans-serif; color: white; font-size: 0.85rem;
...
.ticket-tag.return-tag {
    background: rgba(34,197,94,0.18); border-color: rgba(34,197,94,0.45); color: #4ade80;
}
.ticket-body {
    padding: 28px 32px; display: flex; align-items: center; background: var(--card-bg);
}
.ticket-city { text-align: center; min-width: 100px; flex-shrink: 0; }
.ticket-city .code { font-family: 'Syne', sans-serif; font-size: 2.6rem; font-weight: 900; color: var(--text-h); line-height: 1; }
...
.ticket-mid .dash-line { border-top: 2px dashed var(--card-border); position: relative; height: 24px; display: flex; align-items: center; justify-content: center; }
.ticket-mid .plane { background: var(--card-bg); padding: 0 10px; font-size: 1.15rem; z-index: 1; display: inline-block; margin-top: -12px; }
...
.ticket-footer {
    border-top: 2px dashed var(--card-border); padding: 22px 32px;
...
.route-info-item .val   { font-family: 'Syne', sans-serif; font-size: 0.88rem; font-weight: 800; color: var(--text-h); display: block; }
...
.per-pax-val   { font-family: 'Syne', sans-serif; font-size: 1.45rem; font-weight: 800; color: var(--text-h); }
...
.insight-card-title { font-family: 'Syne', sans-serif; font-size: 1.1rem; font-weight: 900; color: var(--text-h); }
...
.scenario-card-price   { font-family: 'Syne', sans-serif; font-size: 2rem; font-weight: 900; color: var(--primary); margin: 8px 0; }
...
[data-testid="stSidebar"] > div > div, [data-testid="stSidebarContent"] {
    background: #09122c !important; background-color: #09122c !important;
}
...
[data-testid="stSidebar"] div[data-baseweb="select"] > div:first-child {
    ...
    border-radius: 8px !important; color: var(--sidebar-text) !important;
...
[data-testid="stSidebar"] div[data-baseweb="select"] input {
    color: var(--sidebar-text) !important; -webkit-text-fill-color: var(--sidebar-text) !important; opacity: 1 !important;
}
[data-testid="stSidebar"] div[data-baseweb="select"] [class*="indicatorContainer"] svg {
    fill: #cbd5e1 !important; color: #cbd5e1 !important;
}
[data-testid="stSidebar"] div[data-baseweb="menu"] {
    background: #1e2d50 !important; border: 1px solid rgba(255,255,255,0.15) !important;
}
[data-testid="stSidebar"] li[role="option"] {
    color: #e2e8f0 !important; background: #1e2d50 !important; -webkit-text-fill-color: #e2e8f0 !important;
}
[data-testid="stSidebar"] li[role="option"]:hover {
    background: rgba(0,82,204,0.35) !important; color: #93c5fd !important; -webkit-text-fill-color: #93c5fd !important;
}
[data-testid="stSidebar"] hr { border-top: 1px solid rgba(255,255,255,0.08) !important; }
...
.sb-section {
    font-size: 0.62rem !important; font-weight: 700 !important; letter-spacing: 2px !important;
    text-transform: uppercase !important; color: var(--accent) !important;
    margin: 20px 0 10px !important; display: flex; align-items: center; gap: 6px;
}
.sb-section::after { content: ''; flex: 1; height: 1px; background: rgba(245,166,35,0.25); }
.sb-row {
    display: flex; justify-content: space-between; align-items: center;
    padding: 6px 0; border-bottom: 1px solid rgba(255,255,255,0.04); font-size: 0.76rem;
}
...
.scenario-input-label { font-size: 0.65rem; font-weight: 800; color: var(--primary); text-transform: uppercase; letter-spacing: 1.5px; margin-bottom: 8px; }
[data-theme="dark"] .scenario-input-label { color: #93c5fd; }
[data-testid="stExpander"] details summary p,
[data-testid="stExpander"] details summary span {
    color: var(--expander-label) !important; -webkit-text-fill-color: var(--expander-label) !important; opacity: 1 !important; font-weight: 700 !important;
}
[data-testid="stExpander"] details summary svg {
    fill: var(--expander-label) !important; stroke: var(--expander-label) !important; opacity: 1 !important;
}
@media (max-width: 960px) { .page-header::before { display: none; } }
</style>
...
with st.sidebar:
    ...
    if MODEL_LOADED:
        st.markdown(
            ...
            'new_dataset_flight_price_prediction_pipeline.pkl</span></div>',
            unsafe_allow_html=True
        )
    else:
        st.markdown(
    ...
    for k, v in [
        ...
        ('Period',    'New clean dataset'),
        ('Routes',    '30 city pairs'),
        ('Airlines',  '6 carriers'),
        ('Min price', '₹1,105'),
        ('Max price', '₹123,071'),
        ('Avg price', '₹20,890'),
...
if submitted:
    ...
    if show_compare:
        ...
        for al in AIRLINES:
            ...
            diff_v = p - out_price_d
            saving = out_price_d - p
            table_rows.append({
                'Airline':            al,
        ...
        saving_vs_pick = out_price_d - cheapest_price
        ...
        <div class="insight-cards-row">
            ...
            <div class="insight-card blue">
                ...
                <div class="insight-card-value blue">{sym}{out_price_d:,.0f}</div>
            </div>
...
# ═══════════════════════════════════════════════════════════════════════════
#  SCENARIO COMPARISON (unchanged from original)
# ═══════════════════════════════════════════════════════════════════════════
st.divider()
with st.expander('🔀 Scenario Comparison — Compare up to 3 flight configurations', expanded=False):
    ...
    if 'scenarios' not in st.session_state:
        st.session_state.scenarios = [
            {'airline': 'Indigo',    'stops': 'zero', 'dep_hour': 6,  'month': 4, 'weekday': 0, 'label': 'Scenario A'},
            {'airline': 'SpiceJet',  'stops': 'zero', 'dep_hour': 10, 'month': 4, 'weekday': 4, 'label': 'Scenario B'},
            {'airline': 'Air_India', 'stops': 'one',  'dep_hour': 18, 'month': 5, 'weekday': 5, 'label': 'Scenario C'},
        ]
